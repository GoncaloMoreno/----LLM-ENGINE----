...
def get_random_move(board):
    """Gets a random legal move from the current position."""
    legal_moves = list(board.legal_moves)
    if not legal_moves:
        return None
    chosen_move = random.choice(legal_moves)
    return board.san(chosen_move)

def play_one_game(transformer_plays_white, game_num):
    """Plays a single game between the Transformer and Random moves."""
    board = chess.Board()
    ...
    while not board.is_game_over():
        ...
        if is_transformer_turn:
            ...
            print(f"\n--- Game {game_num}: Random's turn ({'White' if board.turn == chess.WHITE else 'Black'}) ---")
            move_san = get_random_move(board)
            if move_san is None:
                print("No legal moves available for Random player.")
                break
            ...
            if board.turn == chess.BLACK:  # If it's now Black's turn, we just made a White move
                ...
                moves.append(move_san)
                current_move_number += 1
            print(f"Random plays: {move_san}")

        clear_output(wait=True) # Optional: for cleaner output in compatible terminals/notebooks
        print(f"Game {game_num} - Move {board.fullmove_number}: {'Transformer' if transformer_plays_white else 'Random'} vs {'Random' if transformer_plays_white else 'Transformer'}")
        print(board)
...
def main():
    ...
    for i in range(NUM_GAMES):
        ...
        print(f"Random plays {'Black' if transformer_plays_white else 'White'}")

        pgn, num_moves, result = play_one_game(transformer_plays_white, game_num)
        game_results.append({
            "GameID": game_num,
        ...
        if game_num % 10 == 0: # Save every 10 games
            ...
            df.to_csv("transformer_vs_random_results.csv", index=False)
            print(f"Results for {game_num} games saved to transformer_vs_random_results.csv")

    df_final = pd.DataFrame(game_results)
    df_final.to_csv("transformer_vs_random_results.csv", index=False)
    print("\nAll games finished. Final results saved to transformer_vs_random_results.csv")
    print(df_final.head())
    ...
    if not df_final.empty:
        ...
        random_wins = len(df_final[((df_final['Transformer_Color'] == 'White') & (df_final['Result'] == '0-1')) |
