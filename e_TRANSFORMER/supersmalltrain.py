...
def train_epoch(model, train_loader, optimizer, criterion, device, scheduler=None, global_step=0, save_dir=None):
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        #     print(f"First sequence: {batch[0].tolist()}")
        #     print(f"Input sequence: {batch[0, :-1].tolist()}")
        #     print(f"Target sequence: {batch[0, 1:].tolist()}")
        #     print("\n")
        ...
        # Print first sequence predictions for debugging
        # if batch_idx == 0:
        #     print("\nFirst sequence predictions:")
        #     print(f"Output shape: {outputs.shape}")
        #     print(f"Output logits for first position: {outputs[0, 0, :10]}")  # Print first 10 logits
        #     print(f"Target token: {target_seq[0, 0]}")
        #     print(f"Predicted token: {outputs[0, 0].argmax().item()}")
        #     print("\n")
        ...
        # if save_dir is not None and global_step % 5000 == 0:
        #     checkpoint_path = save_dir / f'supersmall_step_{global_step}.pt' #f'model_step_{global_step}.pt'
        #     torch.save({
        #         'step': global_step,
...
def main():
    ...
    config = {
        ...
        'd_model': 128,     # Increased from 64 to give more capacity
        'nhead': 8,         # Increased from 4 to match d_model
        'num_decoder_layers': 2,  # Increased from 1 to learn more complex patterns
        'dim_feedforward': 512,   # Increased from 256
        'dropout': 0.1,     # Added back some dropout to prevent overfitting
        'max_seq_length': 600,
        'batch_size': 16,   # Reduced from 32 to focus on fewer sequences
        'num_epochs': 100,
        'learning_rate': 5e-4,  # Reduced from 1e-3 for more stable learning
        'warmup_steps': 100,
    ...
    checkpoint_path = 'E:\LLM_ENGINE\checkpoints\supersmallmodel_epoch_last.pt'  # Update this to your checkpoint
    ...
    train_dataset = my_Dataset(
        in_folder='a_DATA_CLEANUP/supersmall_tokenized',
        idx_folder='a_DATA_CLEANUP/supersmall_maps',
        shuffle=True
    )
    
    val_dataset = my_Dataset(
        in_folder='a_DATA_CLEANUP/supersmall_tokenized',  # Using same data for validation during testing
        idx_folder='a_DATA_CLEANUP/supersmall_maps',
        shuffle=False
    )
    ...
    for epoch in range(config['num_epochs']):
        ...
        if val_loss < best_val_loss:
            ...
            checkpoint_path = save_dir / f'supersmallmodel_epoch_last.pt'
            torch.save({
                'epoch': epoch + 1,
                'step': global_step,
                'model_state_dict': model.state_dict(),
